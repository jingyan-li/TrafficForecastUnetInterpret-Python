...
file_path = "2019-01-14_berlin_9ch96-saliency-target-channel0-W28-24.npy"
...
for i in range(11,12):
    ...
    for i in range(3):
        rect = patches.Rectangle((216, 251), 9, 9, linewidth=0.5, edgecolor="r", facecolor="none")
        axes[i].add_patch(rect)
        axes[i].set_xlim(150,300)
        axes[i].set_ylim(200,350)
    plt.savefig(os.path.join(figure_log_root, os.path.split(file_path)[-1][:-3]+f"-attr-space-startt{start_epoch}.pdf"),
                bbox_inches="tight")
    plt.show()
